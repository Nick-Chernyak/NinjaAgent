...
type DailyCommandExecutor struct {
	handlers map[string]func(chatID int64, ctx context.Context, args string)
}

func (exec DailyCommandExecutor) GetHandler(command string) (func(chatID int64, ctx context.Context, args string), bool) {
	handler, ok := exec.handlers[command]
	return handler, ok
}

func NewDailyCommandExecutor(client *mongo.Client, bot *tgbotapi.BotAPI) *DailyCommandExecutor {
	executor := &DailyCommandExecutor{
